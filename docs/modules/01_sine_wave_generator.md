...
该电路本质上是桥式选频振荡器。起振时，运放提供足够的环路增益，`RC` 网络只允许目标频率附近的信号获得较强正反馈，因此电路最终在该频率点建立稳定振荡。

当输出幅值较小时，二极管支路近似截止，放大倍数略大于稳态所需值，电路能够顺利起振；当输出幅值逐渐增大后，二极管导通，等效反馈电阻减小，闭环增益下降，输出幅值被限制在稳定范围内。因此，该电路同时完成了起振、选频和稳幅三项功能。

### 主要器件作用

- `U5 (LM741CH)`：振荡器的有源放大核心，提供环路增益
- `R4`、`R5`、`C1`、`C2`：构成桥式选频网络，决定振荡频率
- `R1`、`R2`、`R3`：决定放大环节的闭环增益
- `D3`、`D4`：构成非线性稳幅支路，抑制输出继续增大，减轻削顶失真

从作用上看，`RC` 网络决定“振荡在什么频率”，增益网络决定“能不能起振”，二极管支路决定“起振后能否稳定为近似正弦波”。
...
`RC = 1 / (2 π f_0) ≈ 3.18 x 10^-5 s`

取 `C = 0.015 μF`，则有：

`R ≈ 3.18 x 10^-5 / (15 x 10^-9) ≈ 2.12 kΩ`
